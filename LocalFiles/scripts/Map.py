...
class Map:
    def __init__(self, start, goal, map_dim, obs_dim, obs_nums):
        ...
        self.wall_gap = self.map_h/5
        # create borders
        self.borders = self.makeBorders()
    ...
    def makeBorders(self):
        ...
        left_wall = pygame.Rect(11 * self.border_thickness, self.wall_gap, self.border_thickness, self.map_h - self.wall_gap)
        border.append(left_wall)
        right_wall = pygame.Rect(22 * self.border_thickness, 0, self.border_thickness, self.map_h - self.wall_gap)
        border.append(right_wall)
        return border

    def makeObstacles(self):
        self.obstacles = self.borders
        rand_obs = self.makeRandomObs()
        for obs in rand_obs:
...
class MapDraw:
    def __init__(self, start, goal, map_dim):
        self.start = start
        self.goal = goal
        self.map_dim = map_dim
        self.map_h, self.map_w = self.map_dim
    ...
    def drawRobotPos(self, robot_pos):
        pygame.draw.circle(self.map, self.blue, robot_pos, self.node_rad + 5, 0)
